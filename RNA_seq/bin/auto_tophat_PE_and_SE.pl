...
die "$0 <FQ_LIST> <OUTDIR> <SCAFF_FASTA> < MAX_INTRON_LENGTH> <MIXMATCH> <QUALITY_SYSTEM>\n" if (@ARGV != 6);
my ($liblist, $out_dir, $scaff, $max_i_length, $mismatch, $quality_system) = @ARGV;
...
while (<$list>){
	...
	my $read_len;
	my $tophat_para = "-p 4 --max-intron-length $max_i_length -m $mismatch";
    $tophat_para .= " --solexa-quals " if($quality_system == 33);
    $tophat_para .= " --phred64-quals " if($quality_system == 64);
