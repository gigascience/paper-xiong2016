...
while (<$list>)
{
	my ($fq1, $fq2, $inner, $sd) = split /\s+/;
	my $cmd = $cmd_pe;
	my ($id) = $fq1 =~ /([^\/]+)_1.fq$/;
        my $tophat_para = "-p 4 --max-intron-length $max_i_length -m $mismatch";
	$tophat_para .= " -r $inner --mate-std-dev $sd ";
	$tophat_para .= ' --coverage-search --microexon-search ' if length((split("\n", `head -n 2 $fq1`))[1]) >= 50;
	#$tophat_para .= ' --coverage-search ' if length((split("\n", `head -n 2 $fq1`))[1]) >= 75;
	#$tophat_para .= ' --microexon-search ' if length((split("\n", `head -n 2 $fq1`))[1]) >= 50;
